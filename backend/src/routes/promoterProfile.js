...
// tiendas a las que suele ir. Lo puede ver el admin/gerente (cualquier
// promotor) o un supervisor (SOLO los suyos — se valida abajo).
// ---------------------------------------------------------------------------
...
router.get("/:id/profile", async (req, res) => {
  try {
    const { id } = req.params;
    if (req.promoter.role === "supervisor" && !(await promoterBelongsToSupervisor(id, req.promoter.id))) {
